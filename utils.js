...
const coolunicodes = '༗⏻ꐑꗫꕤ꧁꧂꩟꫞𖹭𖹬',
    ran = (() => {
        ...
        function gen(length = 6) { return Array.from({ length }, okay4).join(''); function okay4() { return fromCodePoint(floor(random() * 0x110000)) } }
    ...
    utilMath = (() => {
        const π = Math.PI,
            ...
            { abs } = Math,
            { is } = Object
        return { isInt: Number.isInteger, sanitize, equality, toRad, toDeg, diff, clamp, cycle: Cycle, Cycle }
        function sanitize(num) { return num === +num && num != null && isFinite(num) }
        function equality(𐌏, ...target) { return target.every(_is); function _is(o) { return is(o, 𐌏) } }
    ...
    utilString = (() => {
        ...
        function reverse(string) { return string.split('').toReversed().join('') }
    ...
    [local, session] = (() => {
        if ('undefined'.match(RegExp([typeof Storage, typeof localStorage, typeof sessionStorage].join('|'))))
            return Array(2).fill(UNSUPPORTED)
        ...
        function UNSUPPORTED() {
            return new Proxy({}, {
                get() { throw TypeError('Storage is not supported in this environment') },
                set() { throw TypeError('Storage is not supported in this environment') },
                deleteProperty() { throw TypeError('Storage is not supported in this environment') },
                has() { throw TypeError('Storage is not supported in this environment') }
            })

        }
    })(),
    [Resize, Mutation, Intersection,/*Performance,Pressure*/] = (() => {
        if ('undefined'.match(RegExp([typeof ResizeObserver, typeof MutationObserver, typeof IntersectionObserver].join('|'))))
            return Array(3).fill(UNSUPPORTED)
        class ǃ {
            __observer__ = null
        ...
        function UNSUPPORTED() { throw TypeError('One or more observers is not supported in this environment') }
    ...
    Vector2 = (() => {
        ...
        return assign(v = class v {
            ...
            #max
            #min
            constructor(x = 0, y = 0, min = Number.MIN_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER) { if (arguments.length === 1) { y = v.y(x); x = v.x(x) } Object.seal(this); this.#min = min; this.#max = max; this.set(x, y) }
            ...
            set(...numbers) { if (numbers.length === 1) numbers = [...numbers[0]]; const _keys = keys(this); for (let { length } = numbers; length--;)if (_keys[length] in this) this[_keys[length]] = utilMath.clamp(+numbers[length], this.#min, this.#max) }
            pow(vector) { if (!isArray(vector)) vector = from(vector); for (let { length } = this.value; length--;)vector[length] = this[length] ** vector[length]; this.set(vector); return this }
            add(vector) { if (!isArray(vector)) { if (1 in arguments) vector = [vector, arguments[1]]; vector = from(vector) } for (let { length } = this.value; length--;)vector[length] = this[length] + vector[length]; this.set(vector); return this }
            subtract(vector) { if (!isArray(vector)) { if (1 in arguments) vector = [vector, arguments[1]]; vector = from(vector) } for (let { length } = this.value; length--;)vector[length] = this[length] - vector[length]; this.set(vector); return this }
            multiply(vector) { if (!isArray(vector)) { if (1 in arguments) vector = [vector, arguments[1]]; vector = from(vector) } for (let { length } = this.value; length--;)vector[length] = this[length] * vector[length]; this.set(vector); return this }
            divide(vector) { if (!isArray(vector)) { if (1 in arguments) vector = [vector, arguments[1]]; vector = from(vector) } for (let { length } = this.value; length--;)vector[length] = this[length] / vector[length]; this.set(vector); return this }
            ...
            get magnitude() { return this.value.reduce(reduce); function reduce(a, b) { return abs(a + b) } }
        ...
        function x(vectorLike) { return vectorLike.x ?? vectorLike[0] ?? values(vectorLike)[0] }
        function y(vectorLike) { return vectorLike.y ?? vectorLike[1] ?? values(vectorLike)[1] }
        ...
        function difference(vector, vector2) { if (!isArray(vector)) vector = from(vector); if (!isArray(vector2)) vector2 = from(vector2); const out = from(vector); for (let { length } = out; length--;)out[length] = utilMath.diff(vector2[length], vector[length]); return new v(...out) }
    ...
    color = (() => {
        if ('undefined'.match(RegExp([typeof OffscreenCanvasRenderingContext2D, typeof OffscreenCanvas, typeof CanvasRenderingContext2D].join('|'))))
            return null
        ...
        function ___(e, f = 40) { let $ = parseInt((e = ('' + e).replace(/^#/, "")).substring(0, 2), 16), a = parseInt(e.substring(2, 4), 16), r = parseInt(e.substring(4, 6), 16); return $ = round($ * (1 - f / 100)), a = round(a * (1 - f / 100)), r = round(r * (1 - f / 100)), $ = min(255, max(0, $)), a = min(255, max(0, a)), r = min(255, max(0, r)), "#" + [$, a, r].map(e => { let f = e.toString(16); return 1 === f.length ? "0" + f : f }).join('') }
        function choose() { return '#' + ran.frange(0, 0x1000000).toString(16).padStart(6, 0) }
        ...
        function get(t, prop) { if (supports('color', prop)) { n.fillStyle = prop; return n.fillStyle } if (prop in t) return t[prop]; throw TypeError('CSS does not support the color "' + prop + '"') }
    })()
{
    assign(utilString, {
        toOrdinal,
...
new Vector2
new Color
if (!('undefined'.match(RegExp([typeof Element, typeof HTMLElement, typeof Node].join('|'))))) {
    if (!document.querySelector('head')) throw ReferenceError('You must add a <head> element to this document.')
    { function getIcon() { let 𛱃𛱄𛱅 = document.querySelector("link[rel~='icon']"); if (!𛱃𛱄𛱅) { 𛱃𛱄𛱅 = document.createElement('link'); 𛱃𛱄𛱅.rel = 'icon'; document.head.appendChild(𛱃𛱄𛱅) } return 𛱃𛱄𛱅 } typeof title === 'undefined' && typeof icon === 'undefined' && Object.defineProperties(globalThis, { title: { get() { return document.title }, set(val) { document.title = val } }, icon: { get() { return getIcon().href }, set(val) { getIcon().href = val } } }) }
    ...
    var Elem = class Elem {
        ...
        static attributes = new Set(('accent-height accumulate additive alignment-baseline alphabetic amplitude arabic-form ascent attributeName attributeType azimuth baseFrequency baseline-shift baseProfile bbox #begin bias by calcMode cap-height class clip clip-path clip-rule clipPathUnits color color-interpolation color-interpolation-filters cursor cx cy d decoding descent diffuseConstant direction display divisor dominant-baseline dur dx dy edgeMode elevation end exponent fill fill-opacity fill-rule filter filterUnits flood-color flood-opacity font-family font-size font-size-adjust font-stretch font-style font-variant font-weight fr from fx fy g1 g2 glyph-name glyph-orientation-horizontal glyph-orientation-vertical gradientTransform gradientUnits hanging height horiz-adv-x horiz-origin-x horiz-origin-y href id ideographic image-rendering in in2 intercept k k1 k2 k3 k4 kernelMatrix kernelUnitLength keyPoints keySplines keyTimes lang lengthAdjust letter-spacing lighting-color limitingConeAngle marker-end marker-mid marker-start markerHeight markerUnits markerWidth mask maskContentUnits maskUnits mathematical max media methodExperimental min mode name numOctaves opacity operator order orient orientation origin overflow overline-position overline-thickness paint-order panose-1 path pathLength patternContentUnits patternTransform patternUnits pointer-events points pointsAtX pointsAtY pointsAtZ preserveAlpha preserveAspectRatio primitiveUnits r radius refX refY repeatCount repeatDur requiredFeatures restart result rotateExperimental rx ry scale seed shape-rendering sideExperimental slope spacing specularConstant specularExponent spreadMethod startOffset stdDeviation stemh stemv stitchTiles stop-color stop-opacity strikethrough-position strikethrough-thickness string stroke stroke-dasharray stroke-dashoffset stroke-linecap stroke-linejoin stroke-miterlimit stroke-opacity stroke-width style surfaceScale SVG attribute: crossorigin systemLanguage tabindex tableValues target targetX targetY text-anchor text-decoration text-rendering textLength to transform transform-origin type u1 u2 underline-position underline-thickness unicode unicode-bidi unicode-range units-per-em v-alphabetic v-hanging v-ideographic v-mathematical values vector-effect version vert-adv-y vert-origin-x vert-origin-y viewBox visibility width widths word-spacing writing-mode x x-height x1 x2 xChannelSelector y y1 y2 yChannelSelector z zoomAndPan style xmlns for max min low high optimum target rel preload multiple disabled href autocorrect autocapitalize draggable label innerText textContent dir innerHTML type action method required download style autobuffer value loading name checked src maxLength accept placeholder title controls id columns rows width height frameborder allow').split(' '))
        static { const { prototype } = this; for (const key of Object.getOwnPropertyNames(prototype)) { const descriptor = Object.getOwnPropertyDescriptor(prototype, key); if (typeof descriptor.value === 'function' && key !== 'constructor') { const 우정 = prototype[key]; prototype[key] = (俉俊 => { return 𛰙𛰚𛰪𛰙𛰚; function 𛰙𛰚𛰪𛰙𛰚(...अ) { if (hasOwn(this, Soul)) return 우정.apply(this, अ); return null } })(ERROR_MESSAGE()) } } for (const attribute of this.attributes) { const ǃ = attribute.match(/textContent|innerHTML|innerText/); defineProperty(this.prototype, attribute, { get() { if (hasOwn(this, Soul)) return this.content[attribute] ?? null; return null }, set(val) { if (hasOwn(this, Soul)) if (ǃ && this.childCount) throw TypeError(`You are not allowed to set the "${attribute}" of a parent element`); else { if (this.constructor.name === '__SVG__') this.content.setAttribute(attribute, val); else this.content[attribute] = val } else throw ERROR_MESSAGE() } }) } }
        static animateOnRequestAnimationFrame = new Set
        static requestAnimationFrame = (() => {
            let frame = 0
            const 𝑺𝒍𝒂𝒔𝒉𝒗𝒂𝒍𝒌𝒊𝒆 = globalThis.requestAnimationFrame
            ...
            return function n() {
                ...
                𝑺𝒍𝒂𝒔𝒉𝒗𝒂𝒍𝒌𝒊𝒆(n)
                ++frame
                for (let toAnimate of Elem.animateOnRequestAnimationFrame) try { toAnimate.update(frame) } catch (e) {
                    console.error(e)
        ...
        constructor(opts) {
            ...
            if (!opts || !('tag' in opts) && (!('self' in opts) || !(self instanceof Element))) throw TypeError('Invalid input')//return Elem.error('Cannot create element:missing tag')
            ...
            if (self) {
                if (new.target.elements.has(self.content)) { console.error(self); throw ReferenceError(`Duplicate element not allowed`) } this.content = self; if (this.content === document.body) opts.id = 'body'; else if (this.content === document.documentElement) opts.id = 'html'; else {
                ...
                this.content = this.ǃ(tag); opts.id ??= Elem.#unique
                while (document.getElementById(opts.id)) opts.id = Elem.#unique
            }
            this.content.content = this
            {
                let keys = Object.keys(opts)
                for (let { length } = keys, attr = keys[length - 1]; length--; attr = keys[length])
                    if (new.target.attributes.has(attr))
                        this[attr] = opts[attr]
            }

            if ('text' in opts) this.innerHTML = opts.text
            if (opts.readonly) this.freeze()
            if ('message' in opts) this.innerText = opts.message
            if ('class' in opts) { if (typeof opts.class === 'string') opts.class = opts.class.split(' '); for (let { length } = opts.class; length--;)this.content.classList.add(opts.class[length]) }
            if (events) this.addevent(events)
            if (styles) this.styleMe(styles)
            if (arguments[1] === true) { debugger; opts.parent = body }
            if ('parent' in opts) { if (typeof opts.parent === 'string') opts.parent = new.target.$(opts.parent); this.parent = opts.parent }
            if ('children' in opts) this.children = opts.children
            if ('txt' in opts) this.textContent = opts.txt
            this.#begin(opts)
        }
    ...
    assign(globalThis, { $1: null, _: Elem.$.bind(Elem), __(id) { _(id)?.kill() } })
    var
        { body, html } = globalThis
    class Shadow {
        #me
        ...
        let n = new Shadow($({ tag: 'div', parent: html }), `
            ...
            * {
            ...
            .menu {
                ...
                z-index: 9999;
            }`.replace(/\n/g, '').replace(/\~/g, 'center'), { mode: 'closed', clonable: true })
    ...
    function Prompt({ type: e = "text", defaultValue: t, subtext: n = "", text: a = "Prompt", cancelable: r = !1 }) { shadow.killChildren(); let l = { timing: { easing: "ease-in-out", duration: 300, endDelay: 100 }, frames: { opacity: [1, 0], filter: ["", "grayscale(100%)"], transform: ["", "scaleX(0.8) translate(0,-30%)"] } }; return new Promise((o => { let i, s = $({ tag: "div", class: "bg", parent: shadow, display: "grid", style: "position:fixed;top:50%;left:50%;transform-origin:center;transform:translate(-50%,-50%)" }), c = $({ tag: "button", txt: "Confirm", events: { async"click:1"() { i?.noevent("click"), await u.transition(l), o(d.value), s.fadeOut(s.kill) } }, styles: { "background-color": color.lightgreen, "border-color": color.lightgreen } }), d = e instanceof Elem ? e : $({ placeholder: "Type here", tag: "input", type: e, styles: { "text-align": "center", width: "90%", height: "30px", "border-radius": "5px", margin: "5px" } }), p = []; "string" == typeof a ? p.push($({ tag: "h1", text: a.replace(/\n/g, "<br>") })) : a instanceof Elem && p.push(a), "string" == typeof n ? p.push($({ tag: "p", text: n.replace(/\n/g, "<br>") })) : n instanceof Elem && p.push(n), "none" !== e ? p.push(d) : (d.hide(), d.value = "yes"), void 0 !== t && (d.value = t); let u = $({ tag: "div", class: "menu", parent: s, children: [$({ tag: "div", children: p }), c] }); s.fadeIn((() => { "none" !== e ? d.content.focus() : c.content.focus(), d.on.keydown = e => { let { key: t, shiftKey: n } = e; "Enter" !== t || n ? "Escape" === t && i.content.click() : (e.preventDefault(), c.content.click()) } })), c.after = i = $({ tag: "button", events: { async"click:1"() { c.noevent("click"), await u.transition(l), o(null), s.fadeOut(s.kill) } }, styles: { "background-color": color.red, "border-color": color.red }, txt: "Cancel" }), r || i.hide(), u.transition({ timing: { duration: 400, easing: "ease-in-out" }, frames: { transform: ["translateY(-100px) scale(0.8,0.9)", ""], filter: ["blur(10px)", ""], opacity: [0, 1] } }) })) }
    {
        ...
        on(document, {
            'DOMContentLoaded:1': {
                async[t]() {
                    ...
                    } catch { Elem.error('Cutesy font could not be loaded 😞') }
                }
            }[t]
        })
        let arr = [Math, utilArray, utilMath, utilString, ran, assign]; for (let { length } = arr; length--;) { let me = arr[length]; Object.freeze(me) }
        document.head.appendChild(style(`
                    ...
                    @property --secondary{syntax:"<color>";inherits:false;initial-value:${color.secondary};}
                        ...
                        .hidden{display:none;}
                        `)); html.on.click = a; function a({ target: { content } }) { $1 = content ?? null }
    }
}
...
async function getDataUrl(url) {
    ...
    try {
        ...
        Elem.error(`Resource Error: ${url}-${message}`)
        throw ':('
    }
    try { data = await response.blob() } catch ({ message }) {
        Elem.error(`Failed to convert response to blob: ${message}`)
        return ':('
    }
...
async function preload(e = "", ...t) { return function o(e) { console.groupCollapsed("Bulk load:"); for (const e of t) Elem.success(`${link(e)} loaded successfully`); return console.groupEnd(), e }(await Promise.all(t.map((async function n(t) { return function n(e) { if (!e.ok) return Promise.reject(`Request failed with status ${e.status}`); const t = e.headers.get("Content-Type"); if (t) { let n = "text"; return t.includes("application/json") ? n = "json" : t.includes("application/octet-stream") && (n = "blob"), e[n]() } }(await fetch(e + t)) })))) }